...
pub enum Role {
    /// System-level instructions that set the model's behavior and constraints.
    System,
    /// Messages from the human user (questions, commands, feedback).
    User,
    /// Responses generated by the assistant/model.
    Assistant,
    /// Developer-level system messages (OpenAI-specific; hidden from the model's view in some providers).
    Developer,
    /// Results returned by tool execution, linked back to an assistant's [`ToolCall`] via `tool_call_id`.
    Tool,
}
...
///
/// For tool results set `tool_call_id` **in addition to** `content` (which holds the tool's output).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Message {
