...
//! via [`Sessions::set_title`](crate::session::Sessions::set_title) and
//! notifies the caller through a [`PostTurnEvent::TitleSet`].
...
/// from conversation messages, persists it via `Sessions`, and sends a
/// [`PostTurnEvent::TitleSet`] through `tx`.
pub fn spawn_title_enrichment(
    ...
    tokio::spawn(async move {
        ...
        if let Err(e) = sessions.set_title(&session_key, Some(&title)).await {
            warn!(error = %e, session_key, "failed to persist enrichment title");
            return;
        }

        let _ = tx
            .send(PostTurnEvent::TitleSet {
                title: title.clone(),
            })
            .await;
