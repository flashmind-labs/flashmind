//! Slack OAuth authentication tool.
//!
//! Presents the user with a Slack authorization URL, accepts the code back,
//! exchanges it for a user token, and registers the service tools dynamically.
...
/// Auth tool that handles the Slack OAuth2 authorization code flow.
///
/// Only registered when no valid cached token exists. Credentials are
/// provided inline via `SlackConfig`.
pub struct SlackAuthTool {
...
#[async_trait]
impl Tool for SlackAuthTool {
    fn name(&self) -> &str {
        "slack_auth"
    }

    fn description(&self) -> &str {
        "Authenticate with Slack. Call without arguments to get the \
        ...
        match args.code {
            None => {
                ...
                Ok(ToolResult::interrupt(
                    ...
                    Arc::new(OAuthInterrupt {
                        message: format!(
                            "Please visit this URL to authorize Slack access:\n\n{url}\n\n\
            ...
            Some(code) => {
                ...
                Ok(ToolResult::success(
                    ctx.tool_call_id,
                    "Authentication successful. Slack tools are now available.".to_string(),
                ))
            }
        }
    }
    ...
    fn humanize(&self, args: &Value) -> String {
        if args.get("code").is_some() {
            "Completing Slack OAuth authentication".to_string()
        } else {
            "Getting Slack OAuth authorization URL".to_string()
        }
    }
}
...
impl SlackAuthTool {
    fn register_service_tools(&self) -> Result<()> {
        ...
        let client = Arc::new(SlackClient::new(self.config.clone())?);
        let readonly = self.readonly;

        let mut tools: Vec<Arc<dyn Tool>> = vec![
            Arc::new(SlackListChannelsTool {
                client: client.clone(),
            }),
            Arc::new(SlackReadChannelTool {
                client: client.clone(),
            }),
            Arc::new(SlackReadThreadTool {
                client: client.clone(),
            }),
            Arc::new(SlackSearchMessagesTool {
                client: client.clone(),
            }),
        ];

        if !readonly {
            tools.push(Arc::new(SlackSendMessageTool {
                client: client.clone(),
            }));
            tools.push(Arc::new(SlackAddReactionTool {
                client: client.clone(),
            }));
        }
